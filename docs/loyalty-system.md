...
| _2026-08-20_ | Antigravity AI | Fix: sellos Wallet desactualizados | El pass de Google Wallet quedaba "congelado" (mostraba 1/10 cuando la BD tenía 3 sellos). Causa: `getOrCreateLoyaltyObject` hacía `loyaltyobject.get` y **devolvía el objeto existente sin actualizarlo** al generar el save-link, por lo que el token se creaba con datos viejos; además `loyaltyobject.patch` salta con 404 mientras el usuario no ha guardado la tarjeta (los pushes por sello se pierden en silencio hasta ese momento). Fix: `getOrCreateLoyaltyObject` ahora hace **PATCH siempre** del objeto existente con los datos frescos (sellos, barcode, texto, accountName, links) + `notifyPreference: "NOTIFY"`, y solo inserta si no existe. Con esto, al volver a tocar "Agregar a Google Wallet" el objeto se sincroniza al conteo real (3/10) y notifica al dispositivo. Verificado: BD de 3123456789 tenía stamps*count=3 con 3 logs; el fix aplicado y typecheck OK. |
| \_2026-08-20* | Antigravity AI | Fix: 500 "Wallet Object already exists" | Al regenerar el save-link el endpoint fallaba con 500 `Wallet Object {...} already exists`. **Causa raíz real (descubierta con script contra la API real)**: el PATCH del objeto enviaba `notifyPreference: "NOTIFY"` al nivel raíz del body, pero la API de Google lo rechaza con 400 (`Invalid value at 'resource.notify_preference' ... NotificationSettingsForUpdates`) — el campo correcto es **anidado**: `notificationSettingsForUpdates: { notifyPreference: "NOTIFY" }`. Por eso: (1) el PATCH fallaba → el catch intentaba `insert` → objeto ya existe → 500; (2) los pushes por sello (`updateGoogleWalletPass`, que usaba la misma estructura incorrecta) **fallaban en silencio desde el inicio**, por lo que la tarjeta en Wallet nunca se actualizó y quedó en 1/10 aunque la BD tenía 3. Fix: `notificationSettingsForUpdates` anidado en los 2 patches del servicio + fallback defensivo en `getOrCreateLoyaltyObject` (patch → insert → get) para no romper ante errores. Verificado con script real: PATCH con el campo anidado responde OK y deja balance 3 en Google para 606f7647. Typecheck OK. |
| _2026-08-21_ | Antigravity AI | Cartel QR para punto de venta | Generados en `print-assets/` los materiales imprimibles para que los clientes se registren solos desde el mostrador: QR apuntando a `https://www.clubneon.co/club/register` en 3 formatos — `neon-club-qr.png` (2400px, ECC H, apto para impresión grande), `neon-club-qr.svg` (vectorial, cualquier tamaño sin perder calidad) y `neon-club-poster.html` (cartel A5 con marca NEON: título "Obtén tu tarjeta de fidelización", pasos 1-2-3, beneficio "Acumula 10 sellos y llévate 1 raspado gratis" y botón Imprimir; abrir en navegador y Ctrl+P). Añadida dependencia `qrcode` (devDependency) para regenerarlos. Colores del QR: módulos `#090014` sobre blanco para máxima legibilidad de escaneo impreso. |
| _2026-08-21_ | Antigravity AI | Rediseño registro + Mobile | Rediseñada `app/club/register/_page.tsx` alineada al design system: **(1) Marca sin logo real**: monograma "N" en squircle con gradiente primario→acento→secundario (#ff4fd8→#855dff→#37d6ff) + glow, y wordmark NEON CLUB con gradiente; cuando exista el logo real se reemplaza solo el bloque del header. **(2) Estilo**: card `glass-panel` con borde blanco/12 y glow rosa, colores unificados a los tokens (`--primary`, `--secondary`) en vez de los hex viejos off-palette, footer corregido ("Acumula 10 sellos · 1 raspado gratis" en lugar del slogan erróneo "PAGA 10, LLEVA 11"). **(3) Mobile**: `min-h-dvh` (viewport dinámico), padding inferior con `env(safe-area-inset-bottom)`, inputs `text-base` (16px evita el auto-zoom de iOS al enfocar), `inputMode`/`autoComplete` correctos (tel-national, name, email), prefijo fijo "+57" en el campo celular, normalización del teléfono antes de enviar (quita espacios/guiones), spinner en el botón durante loading, glows ambientales decorativos. Typecheck/lint OK. |
| _2026-08-21_ | Antigravity AI | Registro teléfono-primero | Rediseñado el flujo de `/club/register` a wizard de 2 pasos usando el celular como llave: **Paso 1** solo pide el número; al continuar llama `GET /api/loyalty/lookup?phone=` — si el cliente ya existe con `passToken` lo redirige **inmediatamente a su tarjeta** (`/club/{token}`), si no existe (404) pasa al paso 2. **Paso 2** muestra el número confirmado en un chip con botón "Cambiar" (volver al paso 1 conservando el valor), pide nombre + email opcional y registra vía `POST /api/loyalty/register` (que reutiliza cliente/pass si el teléfono ya existía). Edge case cubierto: cliente existente sin pass web → prellena su nombre desde el lookup y continúa al paso 2 para generarle el pase. Detalles UX: animación de transición entre pasos (keyframes `stepIn`), spinner en ambos submits, truco anti-autofill de Chrome (`fakeusernameremembered`) para que no autocompleta mal el campo único, se mantiene todo lo mobile del rediseño anterior. Typecheck/lint OK. |
