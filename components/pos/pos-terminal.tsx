...
export function PosTerminal() {
  ...
  const showPostFlavorSteps = Boolean(draft.typeId && draft.sizeId && draft.flavorId);
  ...
  const addCurrentItem = () => {
    ...
    setCart((current) => {
      const matchingItem = current.find((item) => sameSelection(item, currentItem));
      ...
      return current.map((item) =>
        item.id === matchingItem.id
          ? {
              ...
              lineTotal: item.unitPrice * (item.quantity + currentItem.quantity),
            }
          : item,
      );
    });
  ...
  const summarizeItem = (item: OrderItem) => {
    ...
    const typeLabel = productTypes.find((entry) => entry.id === item.typeId)?.label;
    const flavorLabel = flavors.find((entry) => entry.id === item.flavorId)?.name;
  ...
  const getSizeCaption = (sizeCode: string, basePrice: number) => {
    const selectedType = productTypes.find((entry) => entry.id === draft.typeId);
  ...
  return (
    <>
      <div ref={topPageRef} className="grid gap-5 pb-28 xl:grid-cols-[1.2fr_0.8fr] xl:pb-0">
      <div className="space-y-5">
        {cart.length > 0 ? (
          <Card className="border-white/10 bg-white/4">
            <CardHeader className="pb-3">
              <CardTitle className="text-base">Productos ya agregados</CardTitle>
            </CardHeader>
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            {showFlavorStep ? (
              <div ref={flavorStepRef} className="scroll-mt-36 md:scroll-mt-28">
              <StepBlock
                step="3"
                title="Sabor activo"
            ...
            {showPostFlavorSteps ? (
              <div ref={postFlavorRef} className="scroll-mt-36 md:scroll-mt-28">
              <>
                <StepBlock
                  step="4"
                  title="Extras"
                ...
                <StepBlock
                  ...
                  items={(Object.keys(paymentLabels) as PaymentMethod[]).map(
                    (method) => ({
                      ...
                      caption: method === "cash" ? "Default" : "Tap para elegir",
              ...
              <div className="rounded-3xl border border-white/10 bg-white/4 p-4">
                <p className="text-muted text-sm">
                  Completa tipo, tamaño y sabor para habilitar extras, pago y el resumen final.
                </p>
              </div>
            )}
          </CardContent>
          ...
          <CardContent>
            {showPostFlavorSteps ? (
              ...
              <div className="rounded-2xl border border-white/10 bg-white/4 p-3">
                ...
                <p className="mt-1 text-xl font-semibold">{cart.length}</p>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/4 p-3">
                <p className="text-muted text-[0.7rem] tracking-[0.18em] uppercase">
                  Piezas
              ...
              <div className="col-span-2 rounded-2xl border border-primary/20 bg-primary/10 p-3 md:col-span-1">
                <p className="text-muted text-[0.7rem] tracking-[0.18em] uppercase">
                  Total estimado
                </p>
                    ...
                    <div className="mt-3 space-y-3">
                      <div>
                        <p className="font-semibold">
                          {currentSummary?.sizeLabel} {currentSummary?.typeLabel}
                        </p>
                        <p className="text-muted text-sm">
                      ...
                      <div className="flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-black/15 p-3">
                        <div>
                          ...
                          <p className="text-sm font-semibold">
                            {currentItem.quantity} unidad{currentItem.quantity > 1 ? "es" : ""}
                          </p>
                        </div>
      ...
      {showPostFlavorSteps ? (
        <div className="fixed inset-x-0 bottom-0 z-30 border-t border-white/10 bg-background/95 p-3 backdrop-blur md:hidden">
          ...
          <p className="text-muted mt-2 text-center text-xs">Total: {currency(draftTotal)}</p>
        </div>
      ) : null}
    </>
  );
